...
##
a<- ggplot(spcs_data, aes(y = CTmax, x = Tmean))+
  xlab("Mean ambient temperature (°C)")+ ylab("Critical thermal maximum (°C)")+
  coord_cartesian(ylim = c(-20, 40))+
  geom_smooth(method='lm', formula= y~x, color="darkgray", alpha= 0.5)+
  geom_point(stroke=0, alpha= 0.25, aes(color = Tmean))+
  scale_color_gradient(low= "#005CAF", high="#CB1B45")+
  theme(legend.position = "none")

d<- ggplot(spcs_data, aes(y = CTmin, x = Tmean))+
  xlab("Mean ambient temperature (°C)")+ ylab("Critical thermal minimum (°C)")+
  coord_cartesian(ylim = c(-20, 40))+
  geom_smooth(method='lm', formula= y~x, color="darkgray", alpha= 0.5)+
  geom_point(stroke=0, alpha= 0.25, aes(color = Tmean))+
  scale_color_gradient(low= "#005CAF", high="#CB1B45")+
...
fig_comp1<- annotate_figure(fig_comp1, top= text_grob("Mean ambient temperature= 14°C (N= 1776)", size=11))
fig_comp2<- ggarrange(e,f,nrow=1,labels = c("e","f"))
fig_comp2<- annotate_figure(fig_comp2, top= text_grob("Mean ambient temperature= 22°C (N= 2198)", size=11))
fig_merge<- ggarrange(a,fig_comp1,d,fig_comp2,nrow=2,ncol=2,widths = c(1,2),labels=c("a","","d",""))

ggsave("FavExt.pdf",fig_merge, width = 10, height= 6, units="in")
